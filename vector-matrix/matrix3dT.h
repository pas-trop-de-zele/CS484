...
public:
    ...
    friend matrix3d operator*(const matrix3d &m, const vector3d<T> &v) {
        matrix3d<T> result(m.name_ + " * " + v.name(), 3, {m(0, 0) * v[0] + m(0, 1) * v[1] + m(0, 2) * v[2], m(1, 0) * v[0] + m(1, 1) * v[1] + m(1, 2) * v[2], m(2, 0) * v[0] + m(2, 1) * v[1] + m(2, 2) * v[2]});
        return result;
    }
...
template <typename T>
matrix3d<T>::matrix3d(const std::string &name,
                      int dims)
...
template <typename T>
matrix3d<T> &matrix3d<T>::operator=(T k) { return operator=({k, k, k, k, k, k, k, k, k}); }
...
template <typename T>
T matrix3d<T>::operator()(int row, int col) const { return cols_[col][row]; }
template <typename T>
...
//=================================================================================================
template <typename T>
void matrix3d<T>::name(const std::string &name) { name_ = name; }
template <typename T>
const std::string &matrix3d<T>::name() const { return name_; }
//=================================== LINEAR ALGEBRA ================================
template <typename T>
...
template <typename T>
matrix3d<T> &matrix3d<T>::operator-=(T k) { operator+=(-k); }
template <typename T>
matrix3d<T> &matrix3d<T>::operator*=(T k) { /* TODO */
}
template <typename T>
matrix3d<T> &matrix3d<T>::operator/=(T k) {
    if (abs(k) < epsilon_) {
...
template <typename T>
matrix3d<T> &matrix3d<T>::operator-=(const matrix3d<T> &b) { return operator+=(b * -1); }
//=================================================================================================
template <typename T>
...
matrix3d<T> matrix3d<T>::transpose() const {
    ...
    // matrix3d res(name_ + "Transpose", 3, {m(0, 0), m(1, 0), m(2, 0), m(0, 1), m(1, 1), m(2, 1), m(0, 2), m(1, 2), m(2, 2), m(0, 3), m(1, 3), m(2, 3)});
    matrix3d res(name_ + "Transpose", 3, {m(0, 0), m(0, 1), m(0, 2), m(1, 0), m(1, 1), m(1, 2), m(2, 0), m(2, 1), m(2, 2)});
    return res;
}
template <typename T>
T matrix3d<T>::determinant() const {
    const matrix3d<T> &m = *this;
    T a = m(0, 0) * (m(1, 1) * m(2, 2) - m(1, 2) * m(2, 1));
    T b = m(0, 1) * (m(1, 0) * m(2, 2) - m(1, 2) * m(2, 0));
    T c = m(0, 2) * (m(1, 0) * m(2, 1) - m(1, 1) * m(2, 0));
    return a - b + c;
}
template <typename T>
...
template <typename T>
matrix3d<T> matrix3d<T>::cofactor() const { /* TODO */
    const matrix3d<T> &m = *this;
    matrix3d<T> minor_m = m.minors();
    return matrix3d<T>(name_ + "_Cofactor", 3, {
                                                   minor_m(0, 0),
                                                   -minor_m(1, 0),
                                                   minor_m(2, 0),
                                                   -minor_m(0, 1),
...
matrix3d<T> matrix3d<T>::adjoint() const {
    const matrix3d<T> &m = *this;
    matrix3d<T> cofactor_m = m.cofactor();
    return cofactor_m.transpose();
}
template <typename T>
...
matrix3d<T> matrix3d<T>::identity(int dims) {
    return matrix3d<T>("", dims, {1, 0, 0, 0, 1, 0, 0, 0, 1});
}
template <typename T>
matrix3d<T> matrix3d<T>::zero(int dims) { /* TODO */
}
template <typename T>
